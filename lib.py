...
def getExpiryTime( max_age = None):
    return datetime.datetime.now( pytz.UTC) - \
        datetime.timedelta( seconds = Params.MAX_AGE if max_age is None else max_age)
...
class S3Bucket():
    ...
    def writeStateFile( self, name, ext, body, ord=None):
        self.bucket.put_object( Key=getStateFilename( name, ext, ord), Body=body)
    ...
    def writePingFile( self, hostname, ip, hold):
        self.writeStateFile( hostname, self.PING_EXT, json.dumps( { 'ip' : ip }))
        ...
        if hold and not hold_file:
            self.writeStateFile( hostname, self.HOLD_EXT, json.dumps( { 'ip' : ip }))
                    ...
                    [
                        {
                            ...
                            'ResourceRecordSet' :
                            {
                                ...
                                'Type' : 'A',
                                'TTL' : Params.TTL,
                                'ResourceRecords' : [{ 'Value' : ip if ip else Params.DEFAULT_IP}]
                            }
                        }
                    ]
                })
